...
function construct_HUD_Categories( useLightMode )
	if useLightMode then
		...
		if rangedModes ~= nil then
			hud_ranged_mode = [[       ${keybinds_color}${key_bind_ranged}${options_color}Ranged Set:\cr ${selection_color}${player_current_ranged|Unset}]]
		else
			hud_ranged_mode = ''
		end
		...
		if enmityModes ~= nil then
			hud_enmityMode = [[        ${keybinds_color}${key_bind_enmity}${options_color}Enmity Mode:\cr ${selection_color}${player_current_enmity|Unset}]]
		else
			hud_enmityMode = ''
		end
		if treasureHunter ~= nil then
			hud_treasure_hunter = [[        ${keybinds_color}${key_bind_treasure_hunter}${options_color}TH Mode:\cr ${selection_color}${player_current_treasure_hunter|Unset}]]
		else
			hud_treasure_hunter = ''
		end
		...
		if player.main_job == "COR" then
			hud_luzaf_mode = [[         ${keybinds_color}${key_bind_luzaf_mode}${options_color}Luzaf:\cr ${selection_color}${player_current_luzaf_mode|Unset}]]
		else
			hud_luzaf_mode = ''
		end
		
		if player.main_job == "COR" then
			hud_quickdraw_mode = [[    ${keybinds_color}${key_bind_QD_mode}${options_color}QuickDraw:\cr ${selection_color}${player_current_quickdraw_mode|Unset}]]
		else
			hud_quickdraw_mode = ''
		end
	...
	----------------------------------------------------
		...
		if rangedModes ~= nil then
			hud_ranged_mode = [[\cr 
			        ${keybinds_color}${key_bind_ranged}${options_color}Ranged Set:\cr ${selection_color}${player_current_ranged|Unset}]]
		else
			hud_ranged_mode = ''
		end
		if enmityModes ~= nil then
			hud_enmityMode = [[\cr        
			        ${keybinds_color}${key_bind_enmity}${options_color}Enmity Mode:\cr ${selection_color}${player_current_enmity|Unset}]]
		else
			hud_enmityMode = ''
		end
		...
		if treasureHunter ~= nil then
			hud_treasure_hunter = [[\cr 
         			${keybinds_color}${key_bind_treasure_hunter}${options_color}TH Mode:\cr ${selection_color}${player_current_treasure_hunter|Unset}]]
		else
			hud_treasure_hunter = ''
		end
		if subWeapon ~= nil then
		...
		if player.main_job == "RDM" or player.sub_job == "RDM" then
			hud_convert_mode = [[\cr
			        ${keybinds_color}${key_bind_convert_mode}${options_color}Convert:\cr ${selection_color}${current_convert_mode|Unset}]]
		else
			hud_convert_mode = ''
		end
		...
		if player.main_job == "COR" then
			hud_luzaf_mode = [[\cr
			        ${keybinds_color}${key_bind_luzaf_mode}${options_color}Luzaf:\cr ${selection_color}${player_current_luzaf_mode|Unset}]]
		else
			hud_luzaf_mode = ''
		end
		if player.main_job == "COR" then
			hud_quickdraw_mode = [[\cr
			        ${keybinds_color}${key_bind_QD_mode}${options_color}QuickDraw:\cr ${selection_color}${player_current_quickdraw_mode|Unset}]]
		else
			hud_quickdraw_mode = ''
		end
		...
		if player.main_job == 'SMN' then
			hud_avatar = [[\cr      
		        ${keybinds_color}${key_bind_avatar}${options_color}Avatar:\cr ${avatar_element_color}${toggle_avatar|Unset}]]
			hud_rage = [[\cr
...
function validateTextInformation()
	...
	if avatar ~= nil then
		main_text_hud.avatar_element_color = avatar
	end
	...
	if enmityModes ~= nil then
    	main_text_hud.player_current_enmity = enmityModes.current
    end
	if treasureHunter ~= nil then
    	main_text_hud.player_current_treasure_hunter = treasureHunter.current
    end
    if mainWeapon ~= nil then
	...
	if convertModes ~= nil then
		main_text_hud.toggle_convert_modes = convertModes.current
	end
	...
	if treasureHunter ~= nil then
	    if treasureHunter.value == "OFF" then
	        main_text_hud.player_current_treasure_hunter = const_off
	    elseif treasureHunter.value == "ON" then
	        main_text_hud.player_current_treasure_hunter = const_on
		end
	end
	
	
	if bpModes ~= nil then
	    if bpModes.value == "Normal" then
	        main_text_hud.current_bp_mode = const_off
	    elseif bpModes.value == "AUTO" then
	        main_text_hud.current_bp_mode = const_on
		end
	end
	
	if convertModes ~= nil then
	    if convertModes.value == "OFF" then
	        main_text_hud.current_convert_mode = const_off
	    elseif convertModes.value == "AUTO" then
	        main_text_hud.current_convert_mode = const_on
		end
	end
	...
	if runspeed ~= nil then
		...
		-- elseif runspeed.value == 'AUTO' then
			-- if autorunspeed then
				-- main_text_hud.toggle_movespeed_lock =  const_on
			-- else
				-- main_text_hud.toggle_movespeed_lock =  const_off
