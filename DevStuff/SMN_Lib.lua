...
\cs(128, 128, 128)${key_bind_mburst} \cs(200, 200, 200)Favor Mode:\cr ${player_current_favor}
...
function validateTextInformation()
	...
	if favor.value then
        main_text_hub.player_current_favor = const_on
    else
        main_text_hub.player_current_favor = const_off
    end
...
function precast(spell)
    ...
    if spell.action_type == 'Magic' and buffactive['Silence'] then 
        ...
        add_to_chat(123, '****** ['..spell.name..' CANCELED - Use Echo Drops] ******')        
    end   
	
	--Should stop[character from running long enough to cast
...
-- Needed to play nice with the run checking
...
function updateFavor( favor )   
    if mBurst then
        if use_UI == true then
            ...
            windower.add_to_chat(8,"----- Favor Mode OFF -----")
        end
    else
        if use_UI == true then
            validateTextInformation()
        else
            windower.add_to_chat(8,"----- Nuking Favor Mode ON -----")
        end
    end
    favorOldValue = favor
end
